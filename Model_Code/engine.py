...
def moving_average_1d(x, win=5, mode='reflect'):
    """
    Apply 1D moving average over the time dimension (dim=1) for a 4D tensor [B, T, H, W]
    """
    assert win % 2 == 1, "Window size must be odd for centered MA"
    B, T, H, W = x.shape
    x_reshaped = x.permute(0, 2, 3, 1).reshape(-1, 1, T)  # [B*H*W, 1, T]

    pad = (win // 2, win // 2)
    kernel = torch.ones(1, 1, win, device=x.device) / win
    x_padded = F.pad(x_reshaped, pad, mode=mode)
    x_smoothed = F.conv1d(x_padded, kernel)  # [B*H*W, 1, T]

    return x_smoothed.view(B, H, W, T).permute(0, 3, 1, 2)  # back to [B, T, H, W]

def qm_loss1(
    forecast,       # [B, T, H, W]
    reference,      # [B, T, H, W]
    sigma=0.2,      # controls tail emphasis in quantile weighting
    lambda_mae=0, # weight for time-domain MAE component
    ma_window=15     # window size for MAE smoothing
):
    B, T, H, W = forecast.shape

    # === Quantile setup ===
    quantiles = (torch.arange(0, 101, step=5) / 100.0).to(forecast.device)  # [21]
    w_tau = 1 - 0.1 * torch.exp(-((quantiles - 0.5) ** 2) / (2 * sigma**2))  # [21]
    w_tau = w_tau.view(-1, 1, 1, 1)  # [Q, 1, 1, 1] for broadcasting

    # === Split into 3 seasonal chunks ===
    month_weights = [0.33, 0.33, 0.34]
    months = len(month_weights)
    time_per_chunk = T // months

    total_qm_loss = 0.0
    for i in range(months):
        λ = month_weights[i]
        f_chunk = forecast[:, i * time_per_chunk:(i + 1) * time_per_chunk]
        r_chunk = reference[:, i * time_per_chunk:(i + 1) * time_per_chunk]

        f_q = torch.quantile(f_chunk, quantiles, dim=1)  # [Q, B, H, W]
        r_q = torch.quantile(r_chunk, quantiles, dim=1)

        error = torch.abs(f_q - r_q)
        weighted_error =error #  w_tau * error
        total_qm_loss += λ * weighted_error.mean()

    # === MAE on moving average time series ===
    forecast_ma = moving_average_1d(forecast, win=ma_window)
    reference_ma = moving_average_1d(reference, win=ma_window)
    mae_loss = torch.abs(forecast_ma - reference_ma).mean()

    # === Combine both losses ===
    total_loss = (1 - lambda_mae) * total_qm_loss + lambda_mae * mae_loss
    return total_loss
...
def qm_loss(
    ...
    """
    Local quantile matching loss using a centered sliding window of 30 days.
